...
{  
      ...
      if(err) return err;
       ...
       if(err) return FreeGlobals();
		/* TODO - Give the user the option to open en existing file..
		.. for editing as well.						*/
...
void InitHandlerStruct(op_handler_ty *arr)
{
...
void FreeHandlerStruct(op_handler_ty *arr)
{
    size_t i = 0;
    /* as long as did not reach end of struct marked by "\0" in member ".cmd" */
    while( i < MENU_CH_NUM )
...
{  
    ...
    FilePrint("/home/idan/Documents/Dev/Git_WD/c/ws8_files/files/man");
}

/* Function that validates main arguments equal allowed val*/
int ValidateArguments(size_t argc, size_t val)
{
    if(argc != val)
    {
...
int HandleUserInput()
{
  ...
  while (NULL != current_file_name)
  {
    ...
    while( 0 != strcmp(op_h_arr[i].cmd, EOR) && !found_flag)
    {
        ...
        if flag found: */ 
        /* TODO - get rid of magic numbers - why '-1' below? as a reader I..
        .. have to start look through the code, maybe a different name and/or..
        ..comment.	*/
        if(-1 != op_h_arr[i].compare_func(op_h_arr[i].cmd) )
        {
            found_flag = 1;
            ...
            if( 0 == strcmp(op_h_arr[i].cmd, "<") )
            {
                ...
                op_h_arr[i].operation_func(input_buff + skip_flag_len);
            }
            else
            {  
                /* any other func needs to recieve filename as argument */
                op_h_arr[i].operation_func(current_file_name);
            }
        }
        ++i;
    }
...
int ExitLogger(const char *filename)
{
...
int GetInputString(char *buff, int echo, int extract_first_line)
{
...
int FilePreAppend(const char *str)
{ 
    ...
    fseek(current_file_ptr, 0, SEEK_END); /* Set the file position indicator to EOF */
    size_t fileSize = ftell(current_file_ptr);  /* Get the current position of the file */
    fseek(current_file_ptr, 0, SEEK_SET);  /* Reset the file position indicator to SOF */
    ...
    /* The return value of fputs is the number of characters written 
    (excluding the null-terminator) if successful. else - returns EOF */
    if (fputs(str, current_file_ptr) != EOF && fputs(temp, current_file_ptr) != EOF)
    {	/* TODO - remove 'if' if unused...	*/
        /*
        fprintf(stderr, "FilePreAppend Error: %s\n",
        info[FILE_WRITE_ERROR].message);
        if error FileClose();
...
{   
    size_t l_count = FileCountLines(filename);
    size_t i = 0; 
     
    /* Open file*/
    err = FileOpen(filename,"r");
    if(err) return err;	/* TODO - seperate ifs in all code to different lines, 
    if ()
    {
    	then...
    }
...
int ValidateFilePointer(FILE *fp)
{   
...
/* Function to compare user flag with str, returns index found, otherwise -1 */
int CompareFlag(const char* flag)
{     
...
int BuildFilePath(const char* rel_path, const char* filename)
{
