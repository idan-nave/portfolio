...
static void GreetUser(); /*TODO: would not print on others machine!*/
...
{  
      ...
      if(err) return err; /*TODO: One liners are not conventional*/
       ...
       if(err) return FreeGlobals(); /*TODO: One liner*/
       ...
       else if (err) return FreeGlobals();
...
{
    filename; /* to be fixed */ /*TODO: fix*/
    /* Open file*/
    err = FileOpen(current_file_name,"a");
    if(err) return err;/*TODO: One liner*/
    ...
    if (fputs(str, current_file_ptr) != EOF) /*TODO: if convention*/
    {
        fprintf(stderr, "FileAppend Error: %s\n",
        info[FILE_WRITE_ERROR].message);
        if error FileClose();/*TODO: if convention*/
        return FILE_WRITE_ERROR;
    }
        
    /* Close file*/
    err = FileClose();
    if(err) return err;/*TODO: if convention*/
    return NO_ERROR;
}
...
int FilePreAppend(const char *str, const char* filename)
{
    ...
    /*TODO: does not compile with all flags*/
    ...
    char* temp = (char*)malloc(fileSize + 1); /*TODO: does not compile with all flags*/
    if (temp == NULL) /*TODO: if convention*/
    {
    ...
    free(temp);   temp = NULL; /*TODO: line convention*/
...
{  
    ...
    FilePrint("/home/idan/Documents/Dev/Git_WD/c/ws8_files/files/man", "unused"); /*TODO: directory too specific, would not work in end users machine*/
}
...
static int HandleUserInput()
{
  ...
  while (NULL != current_file_name) /*TODO: Nothing, I really liked the way you exit the loop*/
  {
...
static int FileCreate(const char *filename)
{
    ...
    if (0 == IsFileExists(filename)) /*TODO: if convention*/
    {
...
{   
    ...
    while(i < sizeof(invalid_chars)) /*TODO: while convention*/
    {
...
static int BuildFilePath(const char* rel_path, const char* filename)
{
    ...
    if (temp_name == NULL) /*TODO: if convention*/
    {
        /* handle memory allocation error */
    ...
    if(NULL == current_file_name) /*TODO: if convention*/
    {
        fprintf(stderr, "BuildFilePath Error: %s\n",
        info[NULL_STRING_ERROR].message);
        return NULL_STRING_ERROR;
